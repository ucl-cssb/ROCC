...
def run_test(save_path):
    P_DIR = os.path.join(os.path.join(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'ROCC'), 'chemostat_env'), 'parameter_files')
    print(P_DIR)
    param_path = os.path.join(P_DIR, 'single_aux.yaml')
    ...
    pop_scaling = 100000
    env = ChemostatEnv(param_path, single_reward_func, sampling_time,  pop_scaling)
    ...
    plt.hlines([30000], 0, 288*5, color = 'g', label = 'target')
    plt.xlim(left = 0)
    ...
    for i in range(0, N):
        ...
        values = agent.predict(np.array([(min_pop + i*(max_pop-min_pop)/N)/pop_scaling]))[0]

        action = np.argmax(values)

        value_function[i] = np.max(values)
        state_action[i] = action + 1
